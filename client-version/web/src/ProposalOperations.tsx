...
export function FieldTasks({ user, notify }: Props) {
  ...
  const filtered = data.filter(
    (t) =>
      ...
      `${t.title} ${t.agent} ${t.employee_id} ${t.note}`
        .toLowerCase()
  ...
  async function run(action: () => Promise<unknown>, success: string) {
    ...
    try {
      await action();
      await cache.invalidateQueries({ queryKey: ["field-tasks"] });
      notify(success);
    } catch (e: any) {
      setMessage(e.message || "Could not update the task.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="proposal-page">
      <div className="page-header">
        <div>
          <div className="eyebrow">FIELD EXECUTION</div>
          <h1>Tasks & review queue</h1>
          <p>Assign daily field work and track completion for your team.</p>
        </div>
      ...
      <div className="proposal-summary">
        <div>
          <b>{data.filter((t) => t.status !== "DONE").length}</b>
        ...
        <div>
          <b>
            {
              data.filter((t) => t.status !== "DONE" && t.due_date < today())
                .length
            }
          </b>
          <span>Overdue</span>
          ...
          >
            ...
            <label>
              ...
              <input
                ...
                placeholder="Visit assigned outlet"
              />
            </label>
            <label>
              Due date
      ...
      <Panel
        ...
        subtitle={`${filtered.length} matching tasks in your scope`}
      >
        <div className="proposal-task-filters">
          <label>
            Search tasks
            <input
              value={query}
          ...
          <>
            <div className="proposal-task-list">
              {filtered.slice(0, limit).map((t) => (
                <div className="proposal-task" key={t.id}>
                  <div className="proposal-task-icon">
                    <ClipboardCheck size={20} />
                  </div>
                  <div className="proposal-task-copy">
                    <button
                    ...
                    <span>
                      {t.agent} · {t.employee_id} · Due {t.due_date}
                    </span>
                    {t.note && <p>{t.note}</p>}
                  </div>
                  <Badge value={t.status} />
                  {t.status !== "DONE" && (
                    <div className="proposal-task-actions">
                      {t.status === "OPEN" && (
                        <button
                          disabled={busy}
                          onClick={() =>
                      ...
                      <button
                        ...
                        onClick={() =>
                          run(
                            () =>
                              patch(`/field-tasks/${t.id}`, { status: "DONE" }),
                            "Task completed",
                          )
                        }
                      >
            ...
            {filtered.length > limit && (
              ...
              >
                Show more tasks · {filtered.length - limit} remaining
              </button>
            )}
          </>
        ) : (
      ...
      {data.find((t) => t.id === selectedId) && (
        <Drawer title="Task details" onClose={() => setSelectedId("")}>
          <DetailList
            data={Object.fromEntries(
              [
                ...
                key,
                data.find((t) => t.id === selectedId)![key],
              ]),
            )}
          />
        </Drawer>
...
export function Incentives({ user, notify }: Props) {
  ...
  async function run(action: () => Promise<unknown>, success: string) {
    ...
    try {
      await action();
      await cache.invalidateQueries({ queryKey: ["incentives"] });
      notify(success);
    } catch (e: any) {
      setMessage(e.message || "Could not save incentives.");
    } finally {
      setBusy(false);
    }
  }
  async function importFile(file?: File) {
    ...
    if (file.size > 2_000_000) {
      setMessage("File must be 2 MB or smaller.");
      return;
    }
    ...
    await run(
      () => post("/incentives/import", { filename: file.name, content_base64 }),
      "Incentive file imported",
    );
  }
  if (isPending) return <Loading />;
  if (error) return <ErrorState error={error} retry={refetch} />;
...
export function Support({ user, notify }: Props) {
  ...
  async function run(action: () => Promise<unknown>, success: string) {
    ...
    try {
      ...
      setErrorText(e.message || "Could not update ticket.");
    } finally {
      setBusy(false);
    }
  }
          ...
          >
            <DetailList
              data={{
                ...
                status: t.status,
                message: t.message,
                response: t.response || "Awaiting response",
              }}
            />
            {errorText && <p role="alert">{errorText}</p>}
            {user.permissions.includes("support.write") &&
              t.status !== "RESOLVED" && (
                <form
                  className="support-response"
                  onSubmit={(e) => {
                    e.preventDefault();
                    run(
                      () =>
                        patch(`/support-tickets/${t.id}`, {
                          status: "RESOLVED",
                ...
                >
                  ...
                  <button
                    className="primary"
                    disabled={busy || (response[t.id] || "").trim().length < 5}
                  >
                    Resolve
                  </button>
                </form>
