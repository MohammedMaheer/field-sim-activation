...
export function FieldTasks({ user, notify }: Props) {
  ...
  return <div className="proposal-page">
    ...
    {canAssign && <Panel title="Assign a task" subtitle="Choose an agent in your authorized team."><form className="proposal-form" onSubmit={e => { e.preventDefault(); run(async () => { await post("/field-tasks", { agent_id: agentId, title, note, due_date: due }); setQuery(title); setStatusFilter("ALL"); setLimit(8); setTitle(""); setNote(""); }, "Task assigned"); }}>
